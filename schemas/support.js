NEWSCHEMA('Support', function(schema) {
    ...
    schema.define('COMPANY_NAME', 'String(100)'); //公司名称
	schema.define('CSRC_NAME', 'String(100)'); //行业名词
    schema.define('SUPPORT', Number);
	...
	schema.setQuery(async function($) {
		...
		var db=DBMS();
		try{
			var res =await db.list('support')
						.autoquery($.query,'id:number,COMPANY_NAME:string,CSRC_NAME:string,SUPPORT:number,status:boolean,created:date,updated:date', 'created_desc')
	...
	schema.setRead(function($) {
		...
		DBMS().find('support').id($.id).sort('created',false).error(404).callback($.callback);
	...
	schema.setInsert(async function($, model) {
        ...
        console.log(model)
        try{
            var setting=await DBMS().insert('support', model).promise();
	...
	schema.setUpdate(async function($, model) {
		...
		// 404 error will be returned if the no records won't be updated
		try{
			var data=await DBMS().modify('support', model).where('id',$.id).promise();
	...
	schema.setRemove(async function($) {
		...
		// 404 error will be returned if the no records won't be updated
		try{
			var data=await DBMS().remove('support').id($.id).promise();
			$.callback({code:200,data:data});
		}catch(err){
			$.callback({code:500,message:'Server Error!'});
		}
    ...
    schema.addWorkflow('findOne',async function($,model){
        var data=await DBMS().find('support').where('CSRC_NAME',model.CSRC_NAME).first().promise();
        $.callback({code: 200,data:data});
    })
