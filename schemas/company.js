NEWSCHEMA('Company', function(schema) {
	schema.define('id', 'number');
	schema.define('COMPANY_ABBR', 'String(100)');
	schema.define('COMPANY_CODE', 'String(100)');
    schema.define('FULL_NAME', 'String(100)');
    schema.define('CSRC_CODE', 'String(100)');
	...
	schema.setQuery(async function($) {
		...
		try{
			var res =await db.list('industry')
						.autoquery($.query,'id:number,COMPANY_ABBR:string,COMPANY_CODE:string,FULL_NAME:string,CSRC_CODE:string,status:boolean,created:date,updated:date', 'created_desc')
